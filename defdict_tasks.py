...
#
# for worker in staff_broken:
#     result[worker[0]] += [worker[1]]
#
...
def best_sender(messages, senders):
    ...
    return sorted([(name, mx) for name, mx in result.items()], key = lambda x:(x[1],x[0]), reverse=True)[0][0]
...
# ###
# from collections import defaultdict
# def best_sender(message: list, senders: list) -> str:
#     """Самый главный спамер"""
#     d = defaultdict(int)
#     for i, v in enumerate(senders):
#         d[v] += len(message[i].split())
